...
The most surprising thing was how often the AI I was using to build this system would suggest something that would make the code less reliable  and then catch itself and acknowledge it. For example, it would propose a feature, note that it could produce negative scores with out of range inputs, and then either fix it or flag it as a known issue. That back and forth made me realize that AI tools just reason through tradeoffs in real time, and the reasoning is just as important as the output. 

**Collaboration**

One instance when it gave a helpful suggestion was when refining my scoring system. It recommeded why certain features should be scored higher, and how more popular also did similar things. It gave good reasoning and overall the suggestion made sense. One instance when it made a bad suggestion was when it made me create a new testing file, when I already had one, and did not realize that it was in a different directory. It then suggested to merge the files and only kept the changes it made, not my tests from the original file. It then ran into some of the errors the original file had tested on already, and then wrote the same tests to fix them.
